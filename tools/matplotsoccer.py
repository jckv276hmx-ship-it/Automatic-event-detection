...
def field(color="green", field_length=None, field_width=None, fig=None, ax=None, figsize=None, show=True):
    if field_length is not None:
        pitch_config["length"] = field_length
    if field_width is not None:
        pitch_config["width"] = field_width
...
def actions(
    ...
    if zoom:
        ...
        xmax = min(mx + d, pitch_config["length"]) + zoompad
        ax.set_xlim(xmin, xmax)
        ymin = max(my - d, 0) - zoompad
        ymax = min(my + d, pitch_config["width"]) + zoompad
