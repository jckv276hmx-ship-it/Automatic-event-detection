...
class Animator:
    ...
    def _build_table_events(events: Optional[pd.DataFrame], main_tracking: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        required_cols = {"timestamp", "ball_control", "duel_players"}
        if not required_cols.issubset(main_tracking.columns):
            return base_events

        duel_rows = main_tracking.loc[main_tracking["ball_control"].eq("duel")].copy()
        if duel_rows.empty:
            return base_events

        table_duels = pd.DataFrame(
            {
        ...
        if "frame_id" in duel_rows.columns:
            table_duels["frame_id"] = duel_rows["frame_id"]
        if "period_id" in duel_rows.columns:
            table_duels["period_id"] = duel_rows["period_id"]

        if base_events is None:
            return table_duels

        all_events = pd.concat([base_events, table_duels], ignore_index=True, sort=False)
        dedup_cols = [c for c in ["frame_id", "event_type", "player_id"] if c in all_events.columns]
        if dedup_cols:
            all_events = all_events.drop_duplicates(subset=dedup_cols, keep="first")
        return all_events
    ...
    def plot_event_table(events: pd.DataFrame, fig: plt.Figure, table_ax: plt.Axes, frame_times: np.ndarray) -> dict:
        ...
        raw_event_types = events.get("event_type")
        if raw_event_types is None and "possession" in events.columns:
            raw_event_types = events["possession"]
        if raw_event_types is None:
            raw_event_types = pd.Series([""] * len(events), dtype=object)
        event_types = raw_event_types.apply(Animator._normalize_event_type_label).to_numpy(dtype=str)

        event_players = events.get("player_id", pd.Series([""] * len(events), dtype=object)).copy()
        if "duel_players" in events.columns:
            duel_mask = events.get("event_type", pd.Series([], dtype=str)).astype(str).eq("duel")
    ...
    def run(self, cmap="jet", vmin=0, vmax=1, max_frames=np.inf, fps=10):
        ...
        if self.show_events:
            ...
            if table_events is not None:
                ...
                events_str = main_tracking.apply(
                    lambda x: (
                        f"{Animator._normalize_event_type_label(x['event_type'])} "
                        f"by {Animator._short_player_label(x['player_id'])}"
                    ),
                    axis=1,
                )
